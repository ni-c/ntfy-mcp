...
export function buildToolFilter(config: {
  ...
  if (allow === undefined) {
    ...
    for (const entry of allow) {
      ...
      const matches = expand(entry, 'NTFY_ALLOW_TOOLS');
      if (matches.length === 0) {
        throw new ToolFilterError(
          `NTFY_ALLOW_TOOLS: no tool matches "${entry}". ` +
      ...
      if (survivors.length === 0) {
        if (entry.endsWith('*')) {
          ...
          console.error(
            `ntfy-mcp: NTFY_ALLOW_TOOLS: "${entry}" matches only write ` +
              'tools, which NTFY_READ_ONLY suppresses — it contributes nothing.'
          );
        ...
        throw new ToolFilterError(
          `NTFY_ALLOW_TOOLS: "${entry}" is a write tool, but NTFY_READ_ONLY ` +
            'is set — it is never registered. Remove it from NTFY_ALLOW_TOOLS, ' +
            `or unset NTFY_READ_ONLY. Available in read-only mode: ${[...READ_TOOLS].sort().join(', ')}.`
        );
      }
  ...
  for (const entry of deny ?? []) {
    ...
    const matches = expand(entry, 'NTFY_DENY_TOOLS');
    if (matches.length === 0) {
      throw new ToolFilterError(
        `NTFY_DENY_TOOLS: no tool matches "${entry}". Valid tools: ${catalogueList()}.`
      );
    }
  ...
  if (selected.size === 0) {
    throw new ToolFilterError(
      suppressedByReadOnly
        ? 'NTFY_ALLOW_TOOLS selects only write tools, but NTFY_READ_ONLY is ' +
            'set — the server would start with an empty tool list.'
        : 'NTFY_ALLOW_TOOLS/NTFY_DENY_TOOLS leave no tools registered — the ' +
